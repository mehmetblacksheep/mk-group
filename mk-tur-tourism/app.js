...
const state={lang:'tr',activeCategory:null,activeItem:null,savedScrollY:0,galleryIndex:0,settings:{whatsappNumber:'905555555555',brandName:'MK Tur Tourism'}};
...
function openWhatsApp(item){const settings=getSettings();const message=`Merhaba, ${item.title} hakkında bilgi almak istiyorum.`;window.open(`https://wa.me/${settings.whatsappNumber}?text=${encodeURIComponent(message)}`,'_blank')}
document.querySelectorAll('.category-card').forEach(btn=>btn.addEventListener('click',()=>selectCategory(btn.dataset.category)));backToCategories.addEventListener('click',()=>{state.activeCategory=null;listSection.classList.add('hidden');categoryGrid.classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})});langSelect.addEventListener('change',e=>setLanguage(e.target.value));closeModal.addEventListener('click',closeModalFn);detailModal.addEventListener('click',e=>{if(e.target===detailModal)closeModalFn()});modalWhatsapp.addEventListener('click',()=>state.activeItem&&openWhatsApp(state.activeItem));whatsappFloat.addEventListener('click',()=>window.open(`https://wa.me/${getSettings().whatsappNumber}`,'_blank'));galleryPrev.addEventListener('click',e=>{e.stopPropagation();if(state.activeItem) renderGallery(state.activeItem,state.galleryIndex-1)});galleryNext.addEventListener('click',e=>{e.stopPropagation();if(state.activeItem) renderGallery(state.activeItem,state.galleryIndex+1)});modalImage.addEventListener('click', () => {
...
async function testSupabaseConnection() {
  ...
  const tours = await fetchItemsFromSupabase('tour', 'tr')

  console.log('Supabase settings:', settings)
  console.log('Supabase tours:', tours)
}
